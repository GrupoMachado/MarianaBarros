...
file_path = "c:/Users/rafae/Desktop/APP/index.html"

with codecs.open(file_path, "r", "utf-8") as f:
    content = f.read()

# Replace NAV array elements
# Remove id: 'sugestao' and id: 'explorar'
# Add id: 'treinos'
nav_regex = r"(const NAV = \[\s*\{ id: 'inicio'[^}]+\},\s*)\{ id: 'sugestao'[^}]+\},\s*\{ id: 'cronograma'[^}]+\},\s*\{ id: 'explorar'[^}]+\},"
replacement_nav = r"\1{ id: 'treinos', label: 'Treinos \u26A1', d: 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10' },\n            { id: 'cronograma', label: 'Meu Desafio \U0001F525', d: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z', requiresAccess: 'desafio30' },"
content = re.sub(nav_regex, replacement_nav, content)

# Inject Global TREINO_MOCK right before function App()
mock_data = """        const TREINO_MOCK = {
            grupos: [
            ...
            return (
                <div className="card" style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '16px', color: 'var(--text-secondary)', fontSize: '14px' }}>
                        ...
                        <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>\\u23F1 {ex.duracao_minutos} min</span>
                    </div>
        ...
        function App() {"""
content = content.replace("        function App() {", mock_data)

# Inject states inside App
state_injection = """            const [page, setPage] = useState('inicio');
            const [selectedGroup, setSelectedGroup] = useState(null);
            const [selectedMuscle, setSelectedMuscle] = useState(null);"""
content = content.replace("            const [page, setPage] = useState('inicio');", state_injection)

# Replace 'explorar'/'sugestao' logic with 'treinos' logic
# Let's replace 'case 'explorar':' section
treino_logic = """                    case 'treinos':
                        if (!selectedGroup) {
                            return (
                                    ...
                                    <div className="grid">
                                        {TREINO_MOCK.grupos.map(g => (
                                            <div key={g.id} className="card" onClick={() => {
                                                ...
                                                <h3 style={{ fontSize: '20px', fontWeight: '800' }}>{g.nome}</h3>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                        ...
                        return (
                            <div style={{ animation: 'fade-in 0.4s ease-out' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '24px' }}>
                                    <button onClick={() => { setSelectedGroup(null); setSelectedMuscle(null); }} style={{ background: 'rgba(255,255,255,0.1)', padding: '12px', borderRadius: '50%', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', border: 'none' }}>
...
case_explorar_regex = r"case 'explorar':[\s\S]*?(?=case 'guardados':)"
content = re.sub(case_explorar_regex, treino_logic + "\n\n                    ", content)

case_sugestao_regex = r"case 'sugestao':[\s\S]*?(?=case 'progresso':)"
content = re.sub(case_sugestao_regex, "", content)

# Change setPage('explorar') to setPage('treinos') where relevant
content = content.replace("setPage('explorar');", "setPage('treinos');")

# Also the direct string 'explorar' if click 'explorar'
content = content.replace("if (n.id === 'explorar')", "if (n.id === 'treinos')")

# Also the Foco de Hoje 'explorar' changes 
content = content.replace("page === 'explorar_senior'", "page === 'treinos_senior'")
